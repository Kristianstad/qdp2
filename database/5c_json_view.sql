-- View: qdp2.v_plan_json

-- DROP VIEW qdp2.v_plan_json;

CREATE OR REPLACE VIEW qdp2.v_plan_json
 AS
WITH lagesmetod AS (
	SELECT l.lage_id,
	...
	GROUP BY b.plan_uuid
         ...
         SELECT jsonb_build_object(
			 ...
			 'features', jsonb_build_array(jsonb_build_object(
				 'id', p_1.plan_uuid, 
				 'type', 'Feature', 
				 'geometry', NULL::unknown, 
				 'bbox', po.bbox, 
				 'properties', jsonb_strip_nulls(jsonb_build_object(
					 'objektidentitet', p_1.plan_uuid, 
					 'versionGiltigFran', p_1.v_giltig_fran::TIMESTAMP WITH TIME ZONE, 
					 'versionGiltigTill', p_1.v_giltig_till::TIMESTAMP WITH TIME ZONE, 
					 'objektversion', p_1.planversion, 
					 'feature:typ', 
					 'detaljplan', 
					 'kommun', p_1.kommun, 
					 'beteckning', p_1.beteckning, 
					 'namn', p_1.namn, 
					 'syfte', p_1.syfte, 
					 'status', p_1.status, 
					 'datumStatusforandring', p_1.datum_statusforandring, 
					 'typ', p_1.typ, 
					 'kvalitetsbeskrivning', k.kvalitet, 
					 'anvandbarhet', p_1.anvandbarhet, 
					 'beskrivningAnvandbarhet', p_1.anvandbarhet_beskrivning, 
					 'plangeometri', po.g_json, 
					 'vertikalAvgransning', p_1.avgransning, 
					 'beslutsinformation', b.beslut, 
					 'planbeskrivning', pb.planbeskrivning, 
					 'planeringsunderlag', u.underlag)))) || bs.bestammelser) AS json,
            p_1.plan_uuid
           FROM qdp2.plan p_1
             LEFT JOIN geometri po ON p_1.plan_uuid = po.g_uuid
             LEFT JOIN beslut b ON p_1.plan_uuid = b.plan_uuid
             LEFT JOIN kvalitet k ON p_1.kval_id = k.kval_id
             LEFT JOIN planbeskrivning pb ON p_1.plan_uuid = pb.plan_uuid
             LEFT JOIN underlag u ON p_1.plan_uuid = u.plan_uuid
             LEFT JOIN bestammelse bs ON p_1.plan_uuid = bs.plan_uuid
        )
 SELECT p.plan_uuid,
    p.objektidentitet,
    p.planversion,
    ...
    j.json::text
   FROM qdp2.v_plan p
     LEFT JOIN json j ON p.plan_uuid = j.plan_uuid;

ALTER TABLE qdp2.v_plan_json
    OWNER TO edit_geodata;
COMMENT ON VIEW qdp2.v_plan_json
    IS 'Plan med json';

GRANT ALL ON TABLE qdp2.v_plan_json TO edit_geodata;
GRANT SELECT ON TABLE qdp2.v_plan_json TO read_geodata;
